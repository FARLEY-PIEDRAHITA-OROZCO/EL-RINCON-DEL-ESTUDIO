@extends('layouts.bases')
@section('title', 'Bases de datos')
@section('content')

<section class="box-container-1">
    <div class="box-1">
        <img src="images/bases-de-datos/guia0.png" alt="">
        <a class="boton" href="https://sena.territorio.la/content/index.php/institucion/Complementaria/institution/SENA/Tecnologia/22810010/Contenido/DocArtic/Guia_de_aprendizaje.pdf" target="_blank"><p class="p-1">GUIA DE APRENDIZAJE</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/material1.png" alt="">
        <a class="boton" href=""><p class="p-1">INTRODUCCIÓN A BASES DE DATOS MYSQL</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/material2.png" alt="">
        <a class="boton" href=""><p class="p-1">MYSQL WORKBENCH</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/guia1.png" alt="">
        <a class="boton" href=""><p class="p-1">ACTIVIDAD DE APRENDIZAJE 1</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/material3.png" alt="">
        <a class="boton" href=""><p class="p-1">ESTUDIO DE CASO IMPORT TECH S.A.S</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/guia2.png" alt="">
        <a class="boton" href=""><p class="p-1">ACTIVIDAD DE APRENDIZAJE 2</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/material4.png" alt="">
        <a class="boton" href=""><p class="p-1">EL LENGUAJE SQL</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/guia3.png" alt="">
        <a class="boton" href=""><p class="p-1">ACTIVIDAD DE APRENDIZAJE 3</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/material5.png" alt="">
        <a class="boton" href=""><p class="p-1">PRUEBAS DE VALIDACIÓN DE DATOS MYSQL</p></a>
    </div>
    <div class="box-1">
        <img src="images/bases-de-datos/guia4.png" alt="">
        <a class="boton" href=""><p class="p-1">ACTIVIDAD DE APRENDIZAJE 4</p></a>
    </div>
</section>

@endsection
